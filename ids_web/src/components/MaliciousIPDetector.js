...
function MaliciousIPDetector() {
    ...
    const handleSubmit = async () => {
      if (!networkData || !badIPs || !labelColumn) {
        alert('Please upload both files and specify the label column.');
        return;
      }
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="mt-6">
          <input
            ...
            className="w-full px-4 py-2 bg-white bg-opacity-20 backdrop-filter backdrop-blur-lg text-white border border-white border-opacity-30 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
            value={labelColumn}
        ...
        {results && (
          <div className="mt-8 bg-white bg-opacity-20 backdrop-filter backdrop-blur-lg shadow-md rounded-lg p-6">
            ...
            <p className="text-white">Total predictions: {results.total_predictions}</p>
            <p className="text-white">Malicious predictions: {results.malicious_predictions}</p>
            <p className="text-white">Flagged IPs: {results.flagged_ips.join(', ')}</p>
            <p className="text-white">Number of flagged IPs: {results.num_flagged_ips}</p>
            <p className="text-white">Accuracy: {results.accuracy.toFixed(2)}</p>
          </div>
        )}
      </div>
    );
